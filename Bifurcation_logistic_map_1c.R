...
for (r in r_list) {
  ...
  for (x in results_nocomplex2) {
    ...
    # three cycles
    poly3 <- logistic_map(poly2, r)
    xpoly <- polynomial(c(0,1))
    period3poly <- poly3 - xpoly
    results3 <- polyroot(period3poly)
    results3_rounded <- round(results3, 3)
    
    results_nocomplex3 <- ifelse(Im(results3_rounded) == 0, as.numeric(results3_rounded), NaN)
    for (x in results_nocomplex3) {
      x_list1 <- c(x_list1, r)
      y_list1 <- c(y_list1, x)
    }
    
    # four cycles
    poly4 <- logistic_map(poly3, r)
    xpoly <- polynomial(c(0,1))
    period4poly <- poly4 - xpoly
    results4 <- polyroot(period4poly)
    results4_rounded <- round(results4, 3)
    
    results_nocomplex4 <- ifelse(Im(results4_rounded) == 0, as.numeric(results4_rounded), NaN)
    for (x in results_nocomplex4) {
      x_list1 <- c(x_list1, r)
      y_list1 <- c(y_list1, x)
    }
}
