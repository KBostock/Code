...
for (r in r_list) {
  ...
  for (i in 3:4) {
    poly_previous <- ifelse(i == 3, poly2, poly_current)
    print(paste("poly_previous is", poly_previous))
    print(paste("poly_curent is", poly_current))
    poly_current <- logistic_map(poly_previous, r)
    print(paste("poly_current is", poly_current))
    period_k_poly <- poly_current - xpoly
    results_k <- polyroot(period_k_poly)
    print(paste("roots of poly are", results_k))
...
# Print results
print(ggplot() + geom_point(aes(x = x_list1, y = y_list1), size = 0.1) + 
          ggtitle("Logistic Map") +
          xlab("r") +
          ylab('X*')+
