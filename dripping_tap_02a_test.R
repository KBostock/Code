...
r_list<- seq(0.2, 0.2, 0.0001) 
iterations_to_not_plot<- 3 
...
for(R in r_list){
  ...
  B <<- x0 -g*M0/k 
  x_t <-function(t) (A*sin(((k/(M0 + R*t))^0.5)*t) + B*cos(((k/(M0 + R*t))^0.5)*t))*exp((-(b + R)/(M0 +R*t))*t) + (M0 + R*t)*g/k - xc 
  
  for (i in 1:iterations_to_not_plot) { # run through non printed points to get to equilibrium
    # print current parameters
    print(paste("For iteration ", i))
    print(paste("A is ", A))
    print(paste("B is ", B))
    print(paste("x0 is ", x0))
    print(paste("v0 is ", v0))
    print(paste("m0 is ", M0))
